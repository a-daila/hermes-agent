...
def _print_setup_summary(config: dict, hermes_home):
    ...
    elif tts_provider == "mimo" and get_env_value("XIAOMI_API_KEY"):
...
def _setup_tts_provider(config: dict):
    ...
    choices.extend(
        [
            ...
            "Xiaomi MiMo TTS (voice design & cloning, style control, needs API key)",
            "NeuTTS (local on-device, free, ~300MB model download)",
            "KittenTTS (local on-device, free, lightweight ~25-80MB ONNX)",
        ]
    )
    providers.extend(["edge", "elevenlabs", "openai", "xai", "minimax", "mistral", "gemini", "mimo", "neutts", "kittentts"])
    ...
    if selected == "neutts":
        ...
        existing = get_env_value("XIAOMI_API_KEY")
        if not existing:
            print()
            print_info("Get an API key at https://platform.xiaomimimo.com/")
            api_key = prompt("Xiaomi API key for MiMo TTS", password=True)
            if api_key:
                save_env_value("XIAOMI_API_KEY", api_key)
                print_success("Xiaomi API key saved")
            else:
                print_warning("No API key provided. Falling back to Edge TTS.")
                selected = "edge"
