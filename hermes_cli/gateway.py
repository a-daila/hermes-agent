...
# the FreeBSD package at /usr/local/etc/rc.d/.  Hermes does NOT generate or
# write them at runtime — there is no per-user scope on FreeBSD, only the
# system one, and editing rc.d / rc.conf requires root.  These helpers
# therefore:
...
#
# The rc.d scripts themselves live in scripts/freebsd/ in the upstream tree
# and are installed by the FreeBSD port via USE_RC_SUBR.
...
def supports_freebsd_rc() -> bool:
    ...
    installed by the package.  We do NOT try to install the rc script
    ourselves — that's the package manager's job."""
    if not is_freebsd():
        return False
...
def freebsd_rc_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    The rc.d script itself is provided by the FreeBSD package; this function
    only flips the rcvar and (re)starts the service.  --system is accepted for
    parity with the systemd dispatcher but is a no-op on FreeBSD (rc.d only
    has system scope).  --run-as-user populates hermes_gateway_user in
    rc.conf so the rc script knows whose ~/.hermes to use.
    """
    ...
    if not FREEBSD_RC_SCRIPT_PATH.exists():
        ...
        print("  The hermes_gateway rc.d script is shipped by the FreeBSD package.")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        # On FreeBSD the rc.d script is shipped by the package — its
        # presence already means "installed".  Whether it's enabled in
        # rc.conf is a separate question handled by sysrc/service.
        return True
    elif is_macos():
